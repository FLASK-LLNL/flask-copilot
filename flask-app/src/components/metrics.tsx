    ...
    return (
      <div
        className={`metrics-grid ${
          visibleCount > 1 ? 'metrics-grid-multi' : 'metrics-grid-single'
        }`}
      >
        {Object.keys(metricDefinitions)
          .filter((key) => visibleMetrics[key as keyof VisibleMetrics])
          .map((metricKey) => {
              ...
              <div key={metricKey} className="metric-card">
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={metricsHistory}>
                    ...
                    <XAxis dataKey="step" stroke="#A78BFA" tick={{ fontSize: 12 }} />
    ...
    if (treeNodes.length > 0) {
      ...
      setMetricsHistory((prev) => {
        ...
        const metricsChanged =
          !lastMetrics ||
          Object.keys(metricDefinitions).some(
            (key) => Math.abs((lastMetrics[key] || 0) - (metrics[key] || 0)) > 0.0001
          ) ||
  ...
  return (
    <div className="card card-padding mt-6">
            ...
            <button
              ...
              onClick={() =>
                setVisibleMetrics((prev) => ({
                  ...prev,
                  [key]: !prev[key as keyof VisibleMetrics],
                }))
              }
      ...
      {metricsHistory.length > 0 ? (
        ...
        <div className="text-tertiary text-center py-8">Waiting for metrics data...</div>
      )}
    </div>
  );
};
