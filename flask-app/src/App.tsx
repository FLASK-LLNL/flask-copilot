...
const ChemistryTool: React.FC = () => {
  ...
  const sendOrchestratorSettingsToBackend = useCallback(
    (settings: OrchestratorSettings): void => {
      if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) {
        return;
      }
  ...
  const handleLocalMcpRequest = useCallback(async (data: WebSocketMessage): Promise<void> => {
    ...
    const response: WebSocketMessageToServer = {
      action: 'local-mcp-response',
      requestId: data.requestId,
      ok: false,
      error: 'Local MCP proxy is not available in this build.',
    };
  ...
  const loadContext = (data: Experiment): void => {
    ...
    setTreeNodes(data.treeNodes ? clearLeafReactions(data.treeNodes) : []);
    setEdges(data.edges ?? []);
  ...
  const reconnectWS = (): void => {
    ...
    socket.onmessage = (event: MessageEvent) => {
      ...
      if (data.type === 'node') {
        ...
        setSelectedTools([]);
      } else if (data.type === 'local-mcp-request') {
