...
def main(no_extras, no_main, editable, git_tag, extras, dry_run):
    """
    Install FLASK-copilot and its key package dependencies without sub-dependencies.
    ...
    click.secho("Installation Helper for FLASK-copilot", fg="cyan", bold=True)
    click.secho("=" * 60, fg="cyan")
    ...
    if git_tag:
        package_location = (
            f"git+https://github.com/FLASK-LLNL/FLASK-copilot.git@{git_tag}"
        )
        editable = False
